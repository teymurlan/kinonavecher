...
function phoneKey(v) {
  return String(v || '').replace(/\D/g, '').replace(/^8(?=\d{10}$)/, '7');
}

function normalizeBootstrap(data) {
  const orders = Array.isArray(data.orders)
    ? data.orders.map(o => ({ ...o, editable: true }))
    : [];
  ...
  const subscriptions = Array.isArray(data.subscriptions)
    ? data.subscriptions.map(s => {
        ...
        return {
          ...
          next: s.next || nextOrder?.date || '',
          comment: s.comment || s.note || ''
        };
      })
    : [];

  return { ...data, orders, subscriptions };
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    const response = await worker.fetch(rewritten, env, ctx);

    if (rewrittenUrl.pathname === '/api/bootstrap' && response.ok) {
      try {
        const data = await response.json();
        return new Response(JSON.stringify(normalizeBootstrap(data)), {
